...
## 🔒 Software Supply Chain Security (SLSA v3)

This project embraces zero-trust and secure-by-default architecture. All release artifacts and container images are cryptographically signed and attested to meet **SLSA Level 3** capabilities entirely via GitHub Actions. 

*   **Keyless Signing:** The official builder image (`ghcr.io/northcutted/resume-ci`) is signed with Sigstore/Cosign via GitHub Action's OIDC identity.
*   **SLSA Provenance:** All resume artifact releases (`resume-artifacts.zip`) include a non-forgeable `intoto.jsonl` provenance document generated by the GitHub Actions Attestation service.
*   **Zero-CVE Base Image:** The builder runs on [Wolfi](https://edu.chainguard.dev/open-source/wolfi/overview/) (cgr.dev/chainguard/wolfi-base) for a minimal, distroless footprint with continuous patch updates.

### Verifying Image Signatures
You can inspect the OIDC-backed signature of the container image using Cosign:
```bash
cosign verify ghcr.io/northcutted/resume-ci:latest \
  --certificate-identity-regexp "https://github.com/northcutted/resume/.*" \
  --certificate-oidc-issuer "https://token.actions.githubusercontent.com"
```

### Verifying Artifact Provenance
You can verify the SLSA provenance attestation using the GitHub CLI after downloading the release archive:
```bash
gh attestation verify resume-artifacts.zip --repo northcutted/resume
```
